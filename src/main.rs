...
fn measure_loop(peripherals: &mut Peripherals) -> Result<()> {
...

    {
        ...
        get_cur_datetime_from_server(&app_config, &mut i2c)?;
        ...
        match measure_sensor_data(
            ...
            Err(e) => {
                let (wifi, mut mqtt_client) =
                    get_wifi_and_mqtt_client(&app_config, &mut peripherals.modem, sysloop.clone())?;
                publish_sensor_error_log_err(&mut mqtt_client, e);
                stop_mqtt_client_and_wifi(wifi, mqtt_client, sysloop.clone())?;
            }
            Ok(m) => {
                if m.light_intensity < 1000. || m.battery_voltage < 3.5 {
                    sleep_dur = std::time::Duration::from_mins(10);
            ...
            if content_len <= 256 {
                ...
                match Read::read(&mut response, &mut buf) {
                    Ok(_size) => {
                        ...
                        ds1307.set_datetime(&datetime.naive_local()).map_err(|e| {
...
fn measure_sensor_data<'m, 'i: 'm>(
    ...
    */
    ...
    fn cur_datetime(ds1307: &mut Ds1307<&mut I2cDriver>) -> anyhow::Result<NaiveDateTime> {
        ds1307
            .datetime()
    ...
    let measure_time = cur_datetime(&mut ds1307)?;
...
fn publish_sensor_error_log_err(client: &mut EspMqttClient, error: anyhow::Error) {
